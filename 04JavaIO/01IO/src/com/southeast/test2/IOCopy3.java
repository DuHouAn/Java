...
public class IOCopy3 {
    public static void main(String[] args) throws IOException {
         for(int i=1;i<=9;i++){
            ...
            File srcFile = new File("D:\\JavaSE\\day0"+i+"\\avi");
            // 封装目的地File
            File destFile = new File("D:\\JavaSE视频\\day0"+i);
            if(!destFile.exists()){
                destFile.mkdir();
            }
        ...
        for(int i=10;i<=27;i++){
            //封装数据源 就是 aaa目录，及其下的所有文件和目录
            File srcFile = new File("D:\\JavaSE\\day"+i+"\\avi");
            // 封装目的地File
            File destFile = new File("D:\\JavaSE视频\\day"+i);
            if(!destFile.exists()){
                destFile.mkdir();
            }
            File[] files=srcFile.listFiles();
